...
use FoxyCart\Services\UtilityService;
...
class OAuth2StoragePDO implements IOAuth2GrantCode, IOAuth2RefreshTokens {
   ...
   * @var string
   */
  const SALT = 'client saltiness of awesomesauce';
  ...
  public function checkClientCredentials($client_id, $client_secret = NULL) {
    try {
    	...
    	//trigger_error('checking credentials: $client_id = ' . $client_id . ' $client_secret = ' . $client_secret);
      ...
      return $this->checkPassword($client_secret, $result['client_secret']);
    } catch (PDOException $e) {
      $this->handleException($e);
    }
  }
  ...
  public function unsetRefreshToken($refresh_token) {
    try {
	  $sql = 'DELETE FROM '.self::TABLE_REFRESH.' WHERE token = :refresh_token';
      $stmt = $this->db->prepare($sql);
   ...
   * @return string
   */
  protected function encrypt($client_secret) {
  	//trigger_error('$client_secret = ' . $client_secret);
  	//trigger_error('encrypted = ' . UtilityService::encrypt($client_secret));
  	$tools = $this->getTools();
  	return $tools::encrypt($client_secret);
  }
  ...
  protected function checkPassword($try, $client_secret) {
  	//trigger_error('check pwd $try = ' . $try . ' $client_secret = ' . $client_secret);
  	return $client_secret == $this->encrypt($try);
  }
}
